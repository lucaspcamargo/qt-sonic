#include "dwsoundrawstream.h"
#include "../nSoundBag.h"
#include <QIODevice>


dwSoundRawStream::dwSoundRawStream(QIODevice *device, nSoundFormat format, int frequency, int channels, QObject *parent)
    : nSoundStream(parent),
      _device(device),
      _format(format),
...
nSoundBag *dwSoundRawStream::createSoundBag(QObject *parent)
{
...
void dwSoundRawStream::rewind()
{
    _device->reset();
}

quint64 dwSoundRawStream::read(void *data, unsigned long frames)
{
