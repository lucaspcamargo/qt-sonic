...
void main() {
    ...
    vec2 fp = fract(tx0.xy * rubyTextureSize);

    vec3 A1 = texture2D(rubyTexture, tx1.xw).rgb;

    vec3 B1 = texture2D(rubyTexture, tx1.yw).rgb;

    vec3 C1 = texture2D(rubyTexture, tx1.zw).rgb;

    vec3 A  = texture2D(rubyTexture, tx2.xw).rgb;

    vec3 B  = texture2D(rubyTexture, tx2.yw).rgb;

    vec3 C  = texture2D(rubyTexture, tx2.zw).rgb;

    vec3 D  = texture2D(rubyTexture, tx3.xw).rgb;

    vec3 E  = texture2D(rubyTexture, tx3.yw).rgb;

    vec3 F  = texture2D(rubyTexture, tx3.zw).rgb;

    vec3 G  = texture2D(rubyTexture, tx4.xw).rgb;

    vec3 H  = texture2D(rubyTexture, tx4.yw).rgb;

    vec3 I  = texture2D(rubyTexture, tx4.zw).rgb;

    vec3 G5 = texture2D(rubyTexture, tx5.xw).rgb;

    vec3 H5 = texture2D(rubyTexture, tx5.yw).rgb;

    vec3 I5 = texture2D(rubyTexture, tx5.zw).rgb;

    vec3 A0 = texture2D(rubyTexture, tx6.xy).rgb;

    vec3 D0 = texture2D(rubyTexture, tx6.xz).rgb;

    vec3 G0 = texture2D(rubyTexture, tx6.xw).rgb;

    vec3 C4 = texture2D(rubyTexture, tx7.xy).rgb;

    vec3 F4 = texture2D(rubyTexture, tx7.xz).rgb;

    vec3 I4 = texture2D(rubyTexture, tx7.xw).rgb;
